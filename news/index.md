...
- [`as_bibentry()`](https://docs.ropensci.org/cffr/reference/as_bibentry.md)
  now returns a `bibentry` object when `x` is `NULL`, using metadata
  from the package in the current working directory.
- [`as_cff.bibentry()`](https://docs.ropensci.org/cffr/reference/as_cff.md)
  now selects the first `issn` of each
  [`bibentry()`](https://rdrr.io/r/utils/bibentry.html).
- [`as_cff.bibentry()`](https://docs.ropensci.org/cffr/reference/as_cff.md)
  now preserves single keywords and generic institutions, removes empty
  keyword values, restores `collection-type: book-series` for **@book**
  and **@inbook** series, preserves legal commas in URLs, removes
  duplicate URLs and extracts DOI resolver URLs only when their paths
  are valid DOI values. Partial BibLaTeX publication dates retain their
  available year and month, while unsupported date values remain
  available for CFF validation. Resolver query and fragment components
  are excluded from DOI values, while original URL metadata is retained.
- `[.cff_ref_lst()` and `[.cff_pers_lst()` now preserve their collection
  classes for subsets of any length.
- [`cff_create()`](https://docs.ropensci.org/cffr/reference/cff_create.md)
  no longer creates duplicate identifiers when merging inferred CRAN
  metadata with `CITATION` metadata.
- [`cff_create()`](https://docs.ropensci.org/cffr/reference/cff_create.md)
  now adds inferred CRAN DOIs only to packages available from CRAN.
  Dependency references retain extracted titles, abstracts, authors and
  citation years while adding their declared scope and version
  constraint, plus URLs and repository information from installed
  package metadata. A release date takes precedence over a citation year
  when available ([\#114](https://github.com/ropensci/cffr/issues/114)).
- [`cff_create()`](https://docs.ropensci.org/cffr/reference/cff_create.md)
  now preserves every recognized alternative license from `DESCRIPTION`
  instead of limiting the CFF output to two SPDX identifiers.
- [`cff_create()`](https://docs.ropensci.org/cffr/reference/cff_create.md)
  now trims, removes empty and duplicate `X-schema.org-keywords` values
  and preserves a single value as a one-item CFF keyword array.
- [`cff_read_cff_citation()`](https://docs.ropensci.org/cffr/reference/cff_read.md)
  ...
  while retaining invalid scalar values for CFF validation.
- [`cff_read_citation()`](https://docs.ropensci.org/cffr/reference/cff_read.md)
  now makes every field supplied in `meta`, including custom
  `DESCRIPTION` fields, available to `CITATION` files and no longer
  retries with metadata from the **base** package when reading fails
  ([\#114](https://github.com/ropensci/cffr/issues/114)).

## cffr 1.4.2

CRAN release: 2026-08-24

- The JOSS paper is now linked as an external **pkgdown** article
  instead of being rebuilt as a package vignette.
- **cli** messages now use more precise semantic markup, pluralization
  and public-facing error contexts.
- [`cff_gha_update()`](https://docs.ropensci.org/cffr/reference/cff_gha_update.md)
  now uses `path` when updating `.Rbuildignore`.
- [`cff_read_bib_text()`](https://docs.ropensci.org/cffr/reference/cff_read_bib_text.md)
  now rejects vectors that mix BibTeX entries with `.bib` file paths.
- [`cff_write_bib()`](https://docs.ropensci.org/cffr/reference/cff_write_misc.md)
  ...
  now stop before modifying an existing file when its backup cannot be
  created, and backup numbering is no longer limited to 100 files.
- [`cff_write()`](https://docs.ropensci.org/cffr/reference/cff_write.md)
  now formats the `inst/CITATION` update message correctly.
...
The API has been completely revised to clarify function naming and
simplify internal maintenance. This change **only** **affects non-core
functions**. Each function now does fewer things but does them better.
...
- [`cff_read()`](https://docs.ropensci.org/cffr/reference/cff_read.md)
  now reads only from external files. It is designed to fit all
  supported file types in a single entry point, along with more specific
  readers used internally by
  [`cff_read()`](https://docs.ropensci.org/cffr/reference/cff_read.md):
  - [`cff_read_cff_citation()`](https://docs.ropensci.org/cffr/reference/cff_read.md).
...
- The **BibTeX** crosswalk was updated (see
  ...
  with corresponding changes in the mapping performed by
  [`toBibtex()`](https://rdrr.io/r/utils/toLatex.html) and
  [`cff_parse_citation()`](https://docs.ropensci.org/cffr/reference/deprecated_cff_bibentry.md):
...
- The **GitHub Actions** workflow now runs only on the `master` or
  `main` branch.
