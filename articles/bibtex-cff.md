...
[**BibTeX**](https://en.wikipedia.org/wiki/BibTeX) is a widely used
format for storing bibliographic references, originally designed in 1985
for document-centric workflows. It represents citations as relatively
flat records with loosely constrained fields, relying on convention
rather than an explicit schema.
...
In modern research workflows, both formats frequently coexist.
**BibTeX** remains dominant in academic publishing pipelines, while CFF
is increasingly adopted by infrastructure platforms such as **GitHub**
and **Zenodo**.
...
models, this mapping is not bijective and may require structural
...
**BibTeX** represents bibliographic information as flat records with
loosely defined fields, optimized for citation rendering in documents.
Semantics are largely implicit and contextual, depending on entry types
...
**BibTeX** entries consist of fields whose relevance and requirements
depend on the entry type. Some fields are mandatory, some are optional
and others are ignored by standard bibliography styles while still
...
between entry types and fields is a defining feature of **BibTeX** and a
key source of friction when interoperating with schema-driven formats
such as CFF.
...
These objects support explicit typing, nested metadata and structured
identifiers, unlike **BibTeX**’s flat field model.
...
require heuristic parsing, especially when normalizing names, dates or
...
| **@inbook** | `type: book` | transform | Distinguished from **@book** by `section` and/or `start`/`end` |
| **@incollection** | `type: generic` | heuristic | `collection-title` preserves the containing work |
...
| **date** | `date-published`, `year` and `month` | transform | Partial dates retain available precision |
| **urldate** | `date-accessed` | transform | Unsupported values are retained for validation |
...
| **doi** | `doi` | direct | \- |
| **url** | `url` and `doi` | split | Resolver URLs also populate `doi` |
...
| **address** | `*.address` or `location.name` | heuristic | Target depends on entry type |
| **institution** | `institution` | transform | Retained unless superseded |
| **organization** | `institution` | transform | Entry-specific normalization |
...
informed by the **Ruby** implementation in Haines and The Ruby Citation
File Format Developers ([2022](#ref-Haines_Ruby_CFF_Library_2021))[^2].

The crosswalk is primarily defined for **BibTeX** to CFF conversion.
Reverse mapping is a best-effort approximation and may be lossy.
...
- **Direct mapping**: one-to-one correspondence between a **BibTeX**
  field and a CFF key.
...
To map general **BibTeX** entries to CFF key `type`, the following
...
- **@book**, **@inbook** and **@incollection** are closely related in
  **BibTeX**[^3]. The crosswalk maps **@book** and **@inbook** to
  `type: book` and distinguishes **@inbook** with `section` and/or
  `start`/`end`. **@incollection** maps to `type: generic`, with
  **booktitle** preserved as `collection-title`. When **series** is
  provided for **@book** or **@inbook**, it is represented by
  `collection-title` together with `collection-type: book-series`.
...
The reverse mapping prioritizes **BibTeX** compatibility and
normalization over exact round-trip preservation.
...
Some **BibTeX** fields and CFF keys have similar names and definitions.
The equivalence is straightforward in some cases, but some keys need to
be processed depending on the **entry** type.
...
| **editor** | `editors` |  |
| **howpublished** | `medium` |  |
| **institution, school, organization** | `institution` | For applicable entries, **school** or **organization** takes precedence when present, and otherwise **institution** is retained. |
| **journal** | `journal` |  |
...
- Some fields are not mapped because they have no clear equivalent CFF
  keys (such as **annote**, **crossref** and **key**). Regarding the
  **type** field, the CFF key `type` corresponds to the identifier of
  the work, similar to an entry in **BibTeX**. Therefore, **BibTeX**
  **type** is not mapped. These fields are always optional in
  **BibTeX**.
- For the **address** field, its intended use in **BibTeX** varies
  depending on the entry type. For **@inproceedings**, it denotes the
  **address** of the **conference**, while for
  **@mastersthesis/@phdthesis**, it is the **address** of the
  **school**. Mapping between **BibTeX** and CFF becomes more complex
  when institutional metadata is involved. This results in varying final
  mappings in CFF. When mapping from CFF to **BibTeX**, we propose to
  follow the same entry-based logic, using the key `location` as a
  fallback value when mapping to **address**.
- Because of the complexity mentioned above, **institution,
  organization** and **school** are mapped to `institution`. For
  applicable entry types, **organization** or **school** takes
  precedence when present, and otherwise an existing **institution** is
  retained.
- A **url** is preserved as `url`. URLs using the `doi.org` or
  `dx.doi.org` resolver also populate `doi` when their paths contain a
  valid DOI value. Each URL is classified independently when an entry
...
- **series** is mapped to `collection-title` only on those entries that
  ...
  **@incollection** and **@inproceedings**, but in those cases it is an
  optional field.
- When mapping from CFF to **BibTeX**, we propose to use
  `date-published` as a fallback for extracting **month** and **year**
  fields.
- When mapping BibLaTeX **date** to CFF, complete dates populate
  `date-published`, while `YYYY` and `YYYY-MM` values populate the
  available **year** and **month** fields without inventing a day. Date
  values that cannot be represented remain unchanged so that CFF
...
Other CFF keys correspond to **BibLaTeX** fields. We include these
fields in the crosswalk[^5], even though they are not part of the core
**BibTeX** field definition.
...
- **Ambiguous semantics**: Certain fields such as **address** and
  **series** have context-dependent meanings in **BibTeX**, requiring
  ...
  `collection-title`.
- **Type collapsing**: Multiple **BibTeX** entry types, including
  **@misc** and **@incollection**, are mapped to a single CFF type
  (`generic`), which acts as a fallback when no more specific type is
  available.
...
- **Non-reversibility**: Because of these factors, round-trip conversion
  (**BibTeX** → CFF → **BibTeX**) is not guaranteed to preserve the
  original structure or semantics.
...
The crosswalk for **@article** uses the default mapping rules.
...
In terms of fields required in **BibTeX**, the primary difference
between **@book** and **@inbook** is that **@inbook** requires a
**chapter** or **page** field, while **@book** does not allow these
fields as optional. Therefore, we propose that an **@inbook** entry in
CFF be treated as a **@book** with the following supplementary fields:
...
There are notable differences in how **BibTeX** and **BibLaTeX** handle
the **@inbook** entry (discussed further in [Appendix
A](#appendix_inbook)). We propose treating a **BibLaTeX** **@inbook** as
...
| **author** | `authors` |  |
| **organization** | `institution` |  |
| **address** | `address` property of `institution` | On missing `organization` mapped to `location` |
| **edition** | `edition` |  |
| **month** | `month` |  |
| **year** | `year` |  |
| **note** | `notes` |  |
...
The **BibTeX** field requirements are identical for **@mastersthesis**
and **@phdthesis**.
...
The crosswalk for **@misc** uses the default mapping rules. This
**entry** does not require any **field**.

Note also that it is mapped to `type: generic`, as with
[**@incollection**](#incol). In this case, **booktitle** is not an
...
prescribe an **author** field. In these cases, because `authors` is
required in CFF, we use *anonymous*[^6] when mapping to CFF and omit it
when mapping from CFF to **BibTeX**.
...
The crosswalk for **@techreport** uses the default mapping rules.
...
The crosswalk for **@unpublished** uses the default mapping rules.
...
**booktitle** requirement in **BibLaTeX**. Notably, **BibTeX**
**@incollection** also requires this field. Moreover, both **BibTeX**
**@incollection** and **BibLaTeX** **@inbook** emphasize its reference
to *“a part of a book (…) with its own title”*.

In this article, the proposed crosswalk ensures full compatibility with
**BibTeX**. Therefore, we treat a **BibLaTeX** **@inbook** entry as
equivalent to a **BibTeX** **@incollection**, given the close alignment
in their definitions and field requirements.
...
[^2]: This software performs only the mapping from CFF to **BibTeX**,
    but **cffr** can perform the mapping in both directions.

[^3]: Note that **BibLaTeX** ([Kime et al. 2025](#ref-biblatexpack))
    handles **@inbook** differently, see [Appendix A](#appendix_inbook).
