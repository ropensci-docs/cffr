...
- outfile:
  ...
  The name and path of the `CITATION.cff` to be created. Relative paths
  are resolved from the current working directory, independently of the
  source specified in `x`.
...
- r_citation:
  ...
  A logical value. If `TRUE`, the R package citation (`inst/CITATION`)
  is created or updated relative to the current working directory. **No
  backup copy is created**. For more control, use
...
For details of `authors_roles` and dependency extraction, see
[`cff_create()`](https://docs.ropensci.org/cffr/reference/cff_create.md).

The `x` argument identifies the metadata source. It does not determine
the output directory. This allows you to create a `CITATION.cff` from a
...
#> ✔ /tmp/Rtmp3vHSaB/file63e2566b34b.cff generated.
#> ══ Validating CFF ══════════════════════════════════════════════════════════════
#> ✔ /tmp/Rtmp3vHSaB/file63e2566b34b.cff is valid.
