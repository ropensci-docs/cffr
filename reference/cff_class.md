...
Under the hood, a
[`cff`](https://docs.ropensci.org/cffr/reference/cff.md) object is a
named [`list`](https://rdrr.io/r/base/list.html) with additional
methods, most notably [`print()`](https://rdrr.io/r/base/print.html) and
...
[`as_cff()`](https://docs.ropensci.org/cffr/reference/as_cff.md)
converts a [`list`](https://rdrr.io/r/base/list.html) to
[`cff`](https://docs.ropensci.org/cffr/reference/cff.md) and removes
elements that are `NULL` or `NA`. Named elements are known as keys in
CFF terminology.

### Subclasses

[cffr](https://CRAN.R-project.org/package=cffr) implements two special
subclasses of [`cff`](https://docs.ropensci.org/cffr/reference/cff.md),
which represent two types of objects defined in the [Citation File
Format
...
- `definitions.reference`: The CFF definition for references to related
  works and other software used in the main work described by the
  `CITATION.cff` file. In
  [cffr](https://CRAN.R-project.org/package=cffr), the subclass
  `cff_ref_lst` represents arrays of `definitions.reference` objects,
  where each element has subclass `cff_ref`.

These subclasses are not complete, valid
[`cff`](https://docs.ropensci.org/cffr/reference/cff.md) objects. They
adapt information to the structures required by the CFF schema.
...
[`utils::person()`](https://rdrr.io/r/utils/person.html).
    ...
    # A subset remains an array.

    one_person <- two_persons[1]
...
### Valid [`cff`](https://docs.ropensci.org/cffr/reference/cff.md) objects

Creating a [`cff`](https://docs.ropensci.org/cffr/reference/cff.md)
object does not ensure its validity against the [Citation File Format
schema](https://github.com/citation-file-format/citation-file-format/blob/main/schema-guide.md):
    ...
    #> == Validating CFF ==============================================================
    #> x Validation failed. This <cff> object has 10 validation errors:
...
A documentation topic describing the `cff` class and subclasses.
