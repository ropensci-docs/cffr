...
- meta:
  ...
  A package metadata object as obtained by
...
- `cff_read_bib()` returns an object of classes
  ...
  as defined by the `definitions.reference` specified in the following
...
`cff_read_citation()` makes every field supplied in `meta`, including
custom `DESCRIPTION` fields, available to the `CITATION` file. When
[`cff_create()`](https://docs.ropensci.org/cffr/reference/cff_create.md)
reads a package `CITATION` file, it constructs `meta` from all fields in
that package's `DESCRIPTION`. With `meta = NULL`, only default encoding
metadata is supplied. If the file cannot be evaluated with those
metadata, `cff_read_citation()` returns `NULL` rather than substituting
metadata from another package.
