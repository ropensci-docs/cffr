...
- dependencies:
  ...
  A logical value. If `TRUE`, add installed package dependencies to the
  `references` CFF key. See **Details**.
...
metadata, `cff_create()` does not retry with metadata from another
package.

When `dependencies = TRUE`, `cff_create()` starts from the first
reference returned by
[`utils::citation()`](https://rdrr.io/r/utils/citation.html) for each
installed dependency. Citation fields such as title, abstract, authors
and year are preserved. Each reference is adapted to the CFF
software-reference type and enriched with the dependency scope and
version constraint from the source `DESCRIPTION`, URLs and repository
information from the installed dependency's `DESCRIPTION` and the
canonical CRAN DOI only when the dependency is available from CRAN. A
release date, when present, determines the year, with the citation year
retained otherwise.
...
- [`vignette("r-cff", package = "cffr")`](https://docs.ropensci.org/cffr/articles/r-cff.md)
  provides details about how the metadata of a package is mapped to
  produce a `cff` object.
